...
void fir::postfix_writer::do_while_node(fir::while_node * const node, int lvl) { 
  ...
  _pf.JZ(mklbl(_whileFinally.back()));

  node->instruction()->accept(this, lvl + 2);
  _pf.JMP(mklbl(_whileCondition.back()));

  _pf.ALIGN();
  _pf.LABEL(mklbl(_whileFinally.back()));

  if (node->finally()) node->finally()->accept(this, lvl + 2);

  _inWhileInit = false;
  _symtab.pop();

  _whileCondition.pop_back();
  _whileFinally.pop_back();
}
...
void fir::postfix_writer::do_function_definition_node(fir::function_definition_node *const node, int lvl) {
  ...
  _currentEpilogueLabel = mklbl(++_lbl);
  _currentFunctionEndLabel = mklbl(++_lbl);
  ...
  _pf.LABEL(_currentFunctionEndLabel);
...
void fir::postfix_writer::do_leave_node(fir::leave_node *const node, int lvl) {
  if ((int)_whileFinally.size() >= node->level()) {
    _pf.JMP(mklbl(_whileFinally[_whileFinally.size() - node->level()]));
  } else
...
void fir::postfix_writer::do_restart_node(fir::restart_node *const node, int lvl) {
  if ((int)_whileCondition.size() >= node->level()) {
    _pf.JMP(mklbl(_whileCondition[_whileCondition.size() - node->level()]));
  } else
    error(node->lineno(), "'restart' outside 'while'");
}
void fir::postfix_writer::do_return_node(fir::return_node *const node, int lvl) {
  if (_inPrologue || _inBlock) {
    _pf.JMP(_currentEpilogueLabel);
  } else
    _pf.JMP(_currentFunctionEndLabel);
  _returnSeen = true;
}
...
void fir::postfix_writer::do_body_node(fir::body_node *const node, int lvl) {
  ...
  _inEpilogue = true;
  _pf.LABEL(_currentEpilogueLabel);
  if (node->epilogue()) {
    _symtab.push();
